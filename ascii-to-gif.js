...
getAsciiFrames(function(asciiFrames) {
  ...
  const allMapLines = asciiFrames.flatMap(getMapLines).map(l => l.replace(ansiRegex, ''));
  const width = 12 * Math.max.apply(Math, allMapLines.map(l => l.replace(/\s+$/, '').length));
  const height = lineHeight * Math.max.apply(Math, asciiFrames.map(f => f.split("\n").length));
